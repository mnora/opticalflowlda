                  ...
                  lda_model* model, lda_suffstats* ss)
{
    ...
    for (n = 0; n < doc->nlocation; n++)
    {
        for (k = 0; k < model->num_topics; k++)
        {
            ss->class_word[k][doc->locations[n]] += doc->count_locations[n]*phi[n][k];
            ss->class_totalw[k] += doc->count_locations[n]*phi[n][k];

            ss->class_angle[k][doc->angles[n]]+=doc->count_locations[n]*phi[n][k];
            ss->class_totala[k]+=doc->count_locations[n]*phi[n][k];
        }
    }
...
void write_word_assignment(FILE* f, document* doc, double** phi, lda_model* model)
{
    ...
    {
        fprintf(f, " %04d:%02d",
                doc->locations[n], argmax(phi[n], model->num_topics));
    }
    fprintf(f, "\n");
    fflush(f);
}
...
void run_em(char* start, char* directory, corpus* corpus)
{
    ...
    int nlocations = max_length;//local locations of a doc
    phi = malloc(sizeof(double*)*nlocations);//phi  topic~location
    for (n = 0; n < nlocations; n++)
	phi[n] = malloc(sizeof(double) * NTOPICS);

    NLOCATIONS = corpus->num_locations;//global locations of a corpus
    eta = malloc(sizeof(double*)*NANGLES);//eta location~angle
    for(m=0; m<NANGLES; m++)
    eta[m] = malloc(sizeof(double*)* NLOCATIONS);
    // initialize model
    ...
    {
        model = new_lda_model(NLOCATIONS, NTOPICS, NANGLES);
    ...
    else if (strcmp(start, "random")==0)
    {
        ...
        lda_mle(model, ss, 0);//0 is the estimate alpha ,when it is 1 ,it needs to use newtons method
        model->alpha = INITIAL_ALPHA;
    }
    else
    {
...
void infer(char* model_root, char* save, corpus* corpus)
{
    ...
    fileptr = fopen(filename, "w");
    sprintf(filename, "%s-word-assignment.dat", save);
...
int main(int argc, char* argv[])
{
    ...
    if (argc > 1)
    {
        if (strcmp(argv[1], "est")==0)
        {
            ...
            NANGLES= atoi(argv[8]);
            corpus = read_data(argv[5],NANGLES);
        ...
        if (strcmp(argv[1], "inf")==0)
        {
            ...
            corpus = read_data(argv[4],atoi(argv[6]));
